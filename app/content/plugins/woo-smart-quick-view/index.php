...
! defined( 'WOOSQ_VERSION' ) && define( 'WOOSQ_VERSION', '1.2.1' );
...
! defined( 'WOOSQ_CHANGELOGS' ) && define( 'WOOSQ_CHANGELOGS', 'https://wordpress.org/plugins/woo-smart-quick-view/#developers' );
...
if ( ! function_exists( 'woosq_init' ) ) {
			...
			class WPcleverWoosq {
				...
				function __construct() {
					...
					if ( get_option( 'woosq_add_to_cart_button', 'single' ) === 'archive' ) {
						add_action( 'woosq_product_summary', 'woocommerce_template_loop_add_to_cart', 25 );
					} else {
				...
				function woosq_quickview() {
					...
					switch ( get_option( 'woosq_content_image', 'all' ) ) {
						case 'product_image':
							if ( has_post_thumbnail() ) {
								$thumb_ids[] = get_post_thumbnail_id( $post_id );
							}
							break;
                    ...
                    <div id="woosq-popup" class="mfp-with-anim">
                        <div class="woocommerce single-product">
                            <div id="product-<?php echo $post_id; ?>" <?php post_class( 'product' ); ?>>
                                ...
                                <div
                                    ...
                                    <div class="summary-content">
										<?php do_action( 'woosq_product_summary' ); ?>
                                    </div>
                                </div>
                            </div>
                        </div>
				...
				function woosq_add_button( $id = null ) {
					if ( ! $id ) {
						global $product;
						$id = $product->get_id();
					}
					if ( $id ) {
						...
						if ( ! empty( $selected_cats ) && ( $selected_cats[0] !== '0' ) ) {
							...
							foreach ( $terms as $term ) {
								if ( in_array( $term, $selected_cats, true ) ) {
									$in_cats = true;
								}
							}
							if ( ! $in_cats ) {
						...
						if ( get_option( 'woosq_button_type', 'button' ) === 'button' ) {
							echo '<button class="woosq-btn woosq-btn-' . esc_attr( $id ) . ' ' . get_option( 'woosq_button_class' ) . '" data-id="' . esc_attr( $id ) . '">' . esc_html( $button_text ) . '</button>';
						} else {
							echo '<a href="#" class="woosq-btn woosq-btn-' . esc_attr( $id ) . ' ' . get_option( 'woosq_button_class' ) . '" data-id="' . esc_attr( $id ) . '">' . esc_html( $button_text ) . '</a>';
						}
					}
				}
				...
				function woosq_shortcode( $atts ) {
					$atts = shortcode_atts( array(
						'id'     => null,
						'type'   => get_option( '_woosq_button_type', 'button' ),
					...
					if ( $atts['id'] ) {
						...
						if ( $atts['type'] === 'link' ) {
							return '<a href="#" class="woosq-btn woosq-btn-' . esc_attr( $atts['id'] ) . ' ' . get_option( 'woosq_button_class' ) . '" data-id="' . esc_attr( $atts['id'] ) . '" data-effect="' . esc_attr( $atts['effect'] ) . '">' . esc_html( $button_text ) . '</a>';
						} else {
							return '<button class="woosq-btn woosq-btn-' . esc_attr( $atts['id'] ) . ' ' . get_option( 'woosq_button_class' ) . '" data-id="' . esc_attr( $atts['id'] ) . '" data-effect="' . esc_attr( $atts['effect'] ) . '">' . esc_html( $button_text ) . '</button>';
						}
					}
				}

				function woosq_admin_menu() {
					add_submenu_page( 'wpclever', esc_html__( 'Smart Quick View', 'woosq' ), esc_html__( 'Smart Quick View', 'woosq' ), 'manage_options', 'wpclever-woosq', array(
						&$this,
                            ...
                            <p>
                                ...
                                <a href="<?php echo esc_url( WOOSQ_REVIEWS ); ?>"
                                   target="_blank"><?php esc_html_e( 'Reviews', 'woosq' ); ?></a> | <a
                                        href="<?php echo esc_url( WOOSQ_CHANGELOGS ); ?>"
                                        target="_blank"><?php esc_html_e( 'Changelogs', 'woosq' ); ?></a>
							...
							<?php if ( $active_tab === 'settings' ) { ?>
                                <form method="post" action="options.php">
                                    ...
                                    <table class="form-table">
                                        ...
                                        <tr>
                                            <th scope="row"><?php esc_html_e( 'Button position', 'woosq' ); ?></th>
                                            <td>
										...
										<span style="color: red">These settings available on Premium Version, click <a
                                                    href="https://wpclever.net/downloads/woocommerce-smart-quick-view"
                                                    target="_blank">here</a> to buy, just $19!</span>
                                            </td>
                                        </tr>
                                        <tr>
				...
				function woosq_row_meta( $links, $file ) {
					...
					if ( $plugin === $file ) {
						$row_meta = array(
							'support' => '<a href="https://wpclever.net/contact" target="_blank">' . esc_html__( 'Premium support', 'woosq' ) . '</a>',
						);
	...
	function woosq_notice_wc() {
		?>
        <div class="error">
            <p><?php esc_html_e( 'WPC Smart Quick View require WooCommerce version 3.0.0 or greater.', 'woosq' ); ?></p>
        </div>
		<?php
	}
}